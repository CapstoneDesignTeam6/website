...
export const HomeView = ({ setTopic }: HomeViewProps) => {
    ...
    const fetchTrending = async () => {
      ...
      try {
        ...
        if (Array.isArray(response) && response.length > 0) {
          ...
          console.warn("API 응답이 비어있거나 예상치 못한 형식입니다. Mock 데이터를 사용합니다.", response);
          debatesToProcess = MOCK_TOPICS; 
        }
      } catch (error) {
        // API 호출 실패 시 mock 데이터 사용
        console.error("트렌딩 토론을 가져오는 데 실패했습니다. MOCK_TOPICS를 사용합니다:", error);
        ...
        const trendingSlides: HeroSlide[] = debatesToProcess.slice(0, 2).map((topicItem, index) => ({
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 md:px-8 py-6 md:py-10 overflow-x-hidden"> {/* 전체 컨테이너 패딩 조정 */}
              ...
              >
                {slide.isAboutAgora ? ( // About Agora 슬라이드 내용
                  <div className="grid lg:grid-cols-2 gap-8 md:gap-12 items-center relative z-10">
                    <div>
                      <span className="inline-block px-3 py-1 bg-primary text-white text-[10px] md:text-sm font-bold rounded-full mb-4 md:mb-6 tracking-widest uppercase">{slide.tag}</span>
                      <h1 className="text-2xl md:text-4xl lg:text-5xl font-extrabold font-headline tracking-tight mb-6 md:mb-8 leading-tight"> {/* 제목 크기 조정 */}
                        {slide.title.split('\n').map((line, idx) => ( // 줄바꿈 처리
                          <span key={idx}>{line}<br/></span>
                        ))}
                      </h1>
                      <p className="text-base md:text-lg text-outline mb-8 md:mb-10 leading-relaxed max-w-xl"> {/* 설명 크기 조정 */}
                        {slide.desc}
                      </p>
                      <div className="flex flex-col sm:flex-row gap-3 md:gap-4"> {/* 버튼 그룹 */}
                        {/* "자세히 보기" 버튼 삭제 */}
                        <button onClick={() => navigate('/setup')} className="px-6 md:px-8 py-3 md:py-4 bg-primary text-white font-bold rounded-xl hover:shadow-lg transition-all text-sm md:text-base">
                  ...
                  <div className="grid lg:grid-cols-2 gap-8 md:gap-12 items-center relative z-10">
                    <div>
                      ...
                      <p className="text-sm md:text-base text-outline mb-8 md:mb-10 leading-relaxed max-w-md"> {/* 설명 크기 조정 */}
                        {slide.desc}
                      </p>
                      <div className="flex flex-col sm:flex-row gap-3 md:gap-4"> {/* 버튼 그룹 */}
                        {/* "자세히 보기" 버튼 삭제 */}
